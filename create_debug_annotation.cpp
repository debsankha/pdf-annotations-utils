...
int main() {
    ...
    newannot->setAuthor({"abc"});
    newannot->setContents({"def"});
    newannot->setUniqueName({"gth"});

    newannot->setFlags(Annotation::Flag::FixedSize);
    ...
    newannot->setStyle(s);
    newannot->style().setOpacity(1.0);
    newannot->style().setColor({250,250,0});
    newannot->style().setWidth(2);


    newannot->setBoundary({0.53, 0.34, 0.36, 0.021});
